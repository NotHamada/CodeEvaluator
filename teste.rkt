  ...
  (let loop ((line-number 1)
    ...
    (cond
      ...
      [(or (is-empty-line? line) (= 1 line-number))
       (loop (add1 line-number) function-lines open-parentesis close-parentesis comments number-of-functions sum-function-lines variables lst-function-lines (read-line input-port))]
      ...
      [else
       (let* (
         ...
         (when (and (= new-open-parentesis new-close-parentesis) (= function-lines 0))
           (loop (add1 line-number) 0 0 0 comments number-of-functions (+ sum-function-lines function-lines) (add1 variables) lst-function-lines (read-line input-port)) 
           )
